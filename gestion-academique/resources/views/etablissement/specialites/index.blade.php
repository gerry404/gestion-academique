{{-- resources/views/etablissement/specialites/index.blade.php --}}
@extends('layouts.app')

@section('title', 'Spécialités - EduManager')
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
    <div class="flex flex-wrap gap-2 flex-1">
        <div class="relative max-w-sm flex-1 min-w-[200px]">
            ...
            <form method="GET" action="{{ route('specialites.index') }}" class="inline">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-4 py-3 font-bold text-brand-700">{{ $specialite->code }}</td>
                    <td class="px-4 py-3 font-medium">{{ $specialite->libelle }}</td>
                    <td class="px-4 py-3">
                        <span class="text-sm">{{ $specialite->departement->libelle ?? '-' }}</span>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        ...
                        <form action="{{ route('specialites.toggle-status', $specialite) }}" method="POST" class="inline">
                            ...
                            @method('PATCH')
                            <button type="submit"
                                    class="w-8 h-8 rounded-lg {{ $specialite->est_actif ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-100 text-green-600 hover:bg-green-200' }} inline-flex items-center justify-center ml-1 transition"
                                    title="{{ $specialite->est_actif ? 'Désactiver' : 'Activer' }}">
                                <i class="fa-solid {{ $specialite->est_actif ? 'fa-pause' : 'fa-play' }} text-xs"></i>
                            </button>
                        </form>
