{{-- resources/views/etablissement/departements/edit.blade.php --}}
@extends('layouts.app')

@section('title', 'Modifier département - EduManager')

@php
    $pageTitle = 'Modifier département';
    $pageSub = 'Mettre à jour les informations du département';
@endphp
            ...
            <div>
                ...
                <select name="chef_departement_id"
                    ...
                    @foreach($personnels as $personnel)
                        <option value="{{ $personnel->id }}" {{ old('chef_departement_id', $departement->chef_departement_id) == $personnel->id ? 'selected' : '' }}>
                            {{ $personnel->prenom }} {{ $personnel->nom }} ({{ $personnel->matricule }})
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="flex items-center gap-3 text-sm">
                    <input type="checkbox" name="est_actif" value="1"
                           {{ old('est_actif', $departement->est_actif) ? 'checked' : '' }}
