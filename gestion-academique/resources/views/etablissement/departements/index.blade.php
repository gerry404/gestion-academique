...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
    <div class="relative max-w-sm flex-1">
        ...
        <form method="GET" action="{{ route('departements.index') }}" class="inline">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-4 py-3">
                        @if($departement->chefDepartement)
                            <span class="text-sm">{{ $departement->chefDepartement->prenom }} {{ $departement->chefDepartement->nom }}</span>
                            <span class="text-xs text-slate-400 block">{{ $departement->chefDepartement->matricule }}</span>
                        @else
                    ...
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        ...
                        <form action="{{ route('departements.toggle-status', $departement) }}" method="POST" class="inline">
                            ...
                            @method('PATCH')
                            <button type="submit"
                                    class="w-8 h-8 rounded-lg {{ $departement->est_actif ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-100 text-green-600 hover:bg-green-200' }} inline-flex items-center justify-center ml-1 transition"
                                    title="{{ $departement->est_actif ? 'Désactiver' : 'Activer' }}">
                                <i class="fa-solid {{ $departement->est_actif ? 'fa-pause' : 'fa-play' }} text-xs"></i>
                            </button>
                        </form>
