{{-- resources/views/inscriptions/edit.blade.php --}}
@extends('layouts.app')

@section('title', 'Modifier inscription - EduManager')

@php
    $pageTitle = 'Modifier inscription';
    $pageSub = 'Mettre à jour les informations de l\'inscription';
@endphp
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
        ...
        <form action="{{ route('inscriptions.update', $inscription) }}" method="POST" class="space-y-4" id="inscriptionForm">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="annee_academique_id"
                    ...
                    @foreach($annees as $annee)
                        <option value="{{ $annee->id }}" {{ old('annee_academique_id', $inscription->annee_academique_id) == $annee->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="departement_id" id="departement_select"
                    ...
                    @foreach($departements as $departement)
                        <option value="{{ $departement->id }}" {{ old('departement_id', $inscription->departement_id) == $departement->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="commentaire" rows="3"
                          ...
                          placeholder="Informations supplémentaires sur l'inscription...">{{ old('commentaire', $inscription->commentaire) }}</textarea>
                @error('commentaire')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(specialite => {
                            ...
                            if (selectedId && specialite.id == selectedId) {
                                ...
                                loadNiveaux(specialite.id, "{{ old('niveau_id', $inscription->niveau_id) }}");
                            }
                            specialiteSelect.appendChild(option);
                        });
                    }
    ...
    const initialSpecialite = "{{ old('specialite_id', $inscription->specialite_id) }}";
    const initialNiveau = "{{ old('niveau_id', $inscription->niveau_id) }}";

    if (initialDepartement) {
        loadSpecialites(initialDepartement, initialSpecialite);
    }

    if (initialSpecialite) {
        loadNiveaux(initialSpecialite, initialNiveau);
    }
});
</script>
@endpush
